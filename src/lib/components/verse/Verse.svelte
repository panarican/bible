<script>
	...
	async function handleSpeak(evt) {
		...
		setTimeout(() => {
			...
			const voice = voices.find(({ lang }) =>
				lang.startsWith($locale === 'en' ? 'en-GB' : $locale === 'es' ? 'es-ES' : $locale)
			);
	...
	function handleFlip(evt) {
		...
		currentResults[currentVerse].heart =
			isFavoritePage || getFavorites().find((value) => value === index) >= 0;
	...
	<div class="back">
		<div class="back__content">
			...
			<div class="button-group">
				...
				<button class="button button--{heart ? 'minus' : 'plus'}" on:click={handleFavorite}
					>Heart</button
				>
			</div>
		</div>
